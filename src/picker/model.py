...
class PickerCNN(nn.Module):
    ...
    The final feature map is flattened — not globally pooled — before the
    linear head. Average-pooling collapses each channel to a single value,
    discarding *where* along the sequence the activity occurred, which is
    exactly the information needed to pinpoint an arrival sample index.
    """
